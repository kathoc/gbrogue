 ...
 * translation unit in BANK0 (the fixed, always-mapped 16KB), exactly like
 * bank0_rng.c. The enqueue functions below are called from banked item
 * logic (BANK2), and call_bank unmaps bank 1 — so a banked caller can only
 * reach code in bank 0. They are pure RAM writes; msgq_flush runs only in
 * the HOME orchestrator (bank 1 mapped) and does the actual render/xp
 * replay, so it may freely call msg_post* / combat_* even from here.
 *
 * --- deferred turn-effect journal ---------------------------------------
 * Banked item logic must not render or grant xp mid-processing: msg_post
 * touches render, and combat_gain_xp both renders and would run under
 * call_bank's interrupt-masked __critical. So the logic only records
 * effects here, and the orchestrator replays them with msgq_flush().
 *
